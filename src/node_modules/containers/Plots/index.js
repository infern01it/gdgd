...
class Plots extends Component {
  ...
  renderPaginate(plotsLength, paginateId) {
    ...
    let allLinks = [];
    const pagiLength = 4;
    if( length <= (pagiLength - 1) ) {
      for( let i = 1 ; i <= length ; i++ ) {
        allLinks.push(i);
      }
    } else if(id + (pagiLength - 1) <= length) {
      for( let i = id ; i <= id + (pagiLength - 1) ; i++ ) {
        allLinks.push(i);
      }
    } else {
      for( let i = length - (pagiLength - 1) ; i <= length ; i++ ) {
        allLinks.push(i);
      }
    }
    const prevLinkClass = classNames('btn btn_arrow_left btn_color_white-f', { 'btn_disabled': id === 1 });
    const nextLinkClass = classNames('btn btn_arrow_right btn_color_white-f', { 'btn_disabled': id === length });
    return (
      <ul className="paginator">
        <li className="paginator__prev">
          {
            id === 1 && <button className={prevLinkClass}>
              <ArrowLeft />
        ...
        <li className="paginator__next">
          {
            id === length && <button className={nextLinkClass}>
              <span>Следующая</span>
              <ArrowRight />
            </button>
          }
