...
import { getCoords, getFilterMap } from 'selectors';
...
class Map extends Component {
    ...
    ymaps.ready(() => {
      ...
      const getFeatureCollection = el => {
        return {
          ...
          geometry: {
            type: 'Point',
            coordinates: getCoords(el.coords)
          },
          properties: {
      ...
      result = points.search(el => getFilterMap(el, filterProps));
  ...
  shouldComponentUpdate(nextProps) {
      if(this.props.filterProps === nextProps.filterProps) {
          return false;
      } else {
          return true;
      }
  ...
  render() {
    return (
      <div className="page-map__ymap map-ymap" id="map"></div>
    );
  }
}
