...
import {updateFilterProps} from 'actions';
...
class AllCity extends Component {
  ...
  componentDidMount() {
    const { urlFilterProps } = this.props;
    if( typeof urlFilterProps.city !== 'undefined' && urlFilterProps.city.length ) {
      this.props.updateFilterProps({ city_id: decodeURIComponent(urlFilterProps.city).split(':') });
      this.updateFilter();
    }
  }
  ...
  openViewArray(id) {
    ...
    this.setState({
      isOpenArray: true,
      isOpenArrayId: id,
      isOpenArrayName: toArr(arrays).filter(el => el.id === id)[0].name
    });
  ...
  render() {
    ...
    const openArray = toArr(arrays).filter(el => el.id === isOpenArrayId);
          ...
          <div className={mapCardsClass}>
            ...
            <div className="map-cards__wrap">
              <div className="map-cards__head">
                {
                  isOpenArray && <Fragment>
                    <Link
                      to={`/array/${isOpenArrayId}`}
                      className="map-cards__title map-cards__title_link h2"
                    >{isOpenArrayName}</Link>
                    {
                      isOpenArray && allArrays.length !== 0 && openArray.length === 0 && <h3 class="map-cards__no-result-open-array">По вашему запросу в данном массиве участков не найдено</h3>
                    }
                    <button
              ...
              <div className="map-cards__body">
                {
                  isOpenArray && allArrays.length !== 0 && openArray.length !== 0 && openArray[0].land_plots.map((element , id) => {
                    return <CardSite
                      key={id}
                      id={element.id}
                      name={element.name}
                ...
                }
                {
                  !isOpenArray && filterArrays.length !== 0 && filterArrays.map((element, id) => {
                    return <CardArray
                      ...
                      name={element.name}
                      area={element.area}
                      priceTotal={element.price_total}
...
const mapStateToProps = (state, ownProps) => ({
  arrays: filteringArrays(state.arrays, state.city, state.filterProps),
  allArrays: toArr(state.arrays),
  city: state.city,
  filterProps: state.filterProps,
  urlFilterProps: ownProps.location.query,
