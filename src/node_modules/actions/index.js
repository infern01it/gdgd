import {
	FETCH_ARRAYS_START,
	FETCH_ARRAYS_SUCCESS,
	FETCH_ARRAYS_FAILURE,
	FETCH_CONCEPT_START,
	FETCH_CONCEPT_SUCCESS,
	FETCH_CONCEPT_FAILURE,
	FETCH_CITY_START,
	FETCH_CITY_SUCCESS,
	FETCH_CITY_FAILURE,
	FETCH_SETTINGS_START,
	FETCH_SETTINGS_SUCCESS,
	FETCH_SETTINGS_FAILURE,
	FETCH_ALL_PAGES_START,
	FETCH_ALL_PAGES_SUCCESS,
	FETCH_ALL_PAGES_FAILURE,
	UPDATE_FILTER_PROPS,
	CLEAR_FILTER_PROPS,
	UPDATE_ALL_CITY_PAGE_PROPS,
	UPDATE_WINDOW_WIDTH_PROPS
} from 'actionTypes';

import {
	fetchArrays as fetchArraysApi,
	fetchConcept as fetchConceptApi,
	fetchCity as fetchCityApi,
	fetchSettings as fetchSettingsApi,
	fetchAllPages as fetchAllPagesApi,
	sendModal as sendModalApi
} from 'api';


/* Получение массива обьектов с массивами из базы */
export const fetchArrays = () => async dispatch => {
	dispatch({type: FETCH_ARRAYS_START});

	try {
		const arrays = await fetchArraysApi();
		dispatch({
			type: FETCH_ARRAYS_SUCCESS,
			payload: arrays
		});
	} catch(err) {
		dispatch({
			type: FETCH_ARRAYS_FAILURE,
			payload: err,
			error: true
		});
	}
}

/* Получение массива обьектов с концепциями из базы */
export const fetchConcept = () => async dispatch => {
	dispatch({type: FETCH_CONCEPT_START});

	try {
		const concept = await fetchConceptApi();
		dispatch({
			type: FETCH_CONCEPT_SUCCESS,
			payload: concept
		});
	} catch(err) {
		dispatch({
			type: FETCH_CONCEPT_FAILURE,
			payload: err,
			error: true
		});
	}
}

/* Получение массива обьектов с городами из базы */
export const fetchCity = () => async dispatch => {
	dispatch({type: FETCH_CITY_START});

	try {
		const city = await fetchCityApi();
		dispatch({
			type: FETCH_CITY_SUCCESS,
			payload: city
		});
	} catch(err) {
		dispatch({
			type: FETCH_CITY_FAILURE,
			payload: err,
			error: true
		});
	}
}

/* Получение настроек сайта из базы */
export const fetchSettings = () => async dispatch => {
	dispatch({type: FETCH_SETTINGS_START});

	try {
		const city = await fetchSettingsApi();
		dispatch({
			type: FETCH_SETTINGS_SUCCESS,
			payload: city
		});
	} catch(err) {
		dispatch({
			type: FETCH_SETTINGS_FAILURE,
			payload: err,
			error: true
		});
	}
}

/* Получение данных всех страниц */
export const fetchAllPages = () => async dispatch => {
	dispatch({type: FETCH_ALL_PAGES_START});

	try {
		const pages = await fetchAllPagesApi();
		dispatch({
			type: FETCH_ALL_PAGES_SUCCESS,
			payload: pages
		});
	} catch(err) {
		dispatch({
			type: FETCH_ALL_PAGES_FAILURE,
			payload: err,
			error: true
		});
	}
}

/* Обновление фильтра */
export const updateFilterProps = props => dispatch => {
	dispatch({
		type: UPDATE_FILTER_PROPS,
		payload: props
	});
}

/* Очистка фильтра */
export const clearFilterProps = () => dispatch => {
	dispatch({
		type: CLEAR_FILTER_PROPS
	});
}

/* Отправка форм */
export const sendModal = props => {
	sendModalApi(props);
}

/* Размер устройства */
export const updateWindowWidth = props => dispatch => {
	dispatch({
		type: UPDATE_WINDOW_WIDTH_PROPS,
		payload: props
	});
}
